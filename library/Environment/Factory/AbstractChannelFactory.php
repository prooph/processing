 ...
 * to select a channel for a given service name with the format processing.(command_bus|event_bus).<target>(___<origin>)(___<sender>)
 *
...
class AbstractChannelFactory implements AbstractFactoryInterface
{
    ...
    {
        return (preg_match('/^processing\.(command|event)_bus\..+/', $requestedName))? true : false;
    }
    ...
    public function getToMessageTranslator()
    {
        if (is_null($this->toMessageTranslator)) {
            $this->toMessageTranslator = new ToProcessingMessageTranslator();
        }
